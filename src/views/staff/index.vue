<template>
    <div class="staff-page">
        <PageHeader title="人員管理">
            <template #action>
                <IconBtn v-if="canManageStaff" icon="PLUS" label="新增使用者" class="btn-primary" @click="handleAdd" />
            </template>
        </PageHeader>

        <SearchToolbar v-model:search-query="searchQuery" v-model:filter-department="filterDepartment"
            v-model:filter-role="filterRole" v-model:filter-status="filterStatus" :show-filters="isAdmin" />

        <StaffTable :users="paginatedUsers" :is-admin="isAdmin" :is-manager="isManager"
            :current-user-department="userStore.user?.department" :sort-field="sortField" :sort-order="sortOrder"
            @edit="handleEdit" @change-role="handleChangeRole" @reset-password="handleResetPassword"
        ...
        <!-- 調整權限 Modal -->
        <ChangeRoleModal v-model="showChangeRoleModal" :user="selectedUser" @submit="handleRoleSubmit" />

        <!-- 重置密碼 Modal -->
        <ResetPasswordModal v-model="showResetPasswordModal" :user="selectedUser" @submit="handlePasswordSubmit" />

        <!-- 刪除確認 Modal -->
        <ConfirmDeleteModal v-model="showDeleteModal"
            :message="`確定要刪除使用者「${selectedUser?.name || selectedUser?.username}」嗎？`" detail="刪除後將無法恢復此使用者資料"
